 ...
 *
 * Javaは Double Dispatch ではなく Single Dispatch である．
 * 実際に レシーバ（メソッドを呼び出されるオブジェクト）やメソッド引数の型の違いによって
 * どのように呼ばれるメソッドが変化するか試してみてください．
 *
 * ※ 講義資料から少し変化させています．
 *
 * @author tkobaya
...
public class Player {
    ...
    void hit(GameObject go) {
        // A
         System.out.println("(A)" + this.getClass().getSimpleName() + "(" + type + ") hit with GameObject (" + go + ")");
     }
 
    // 発展問題： このクラス内の hit(FireFlower) と hit(Turtle)をコメントアウトするとどうなるか？ 実際に動かしてみてなぜそれが起こるか説明してみよう．
    void hit(FireFlower ff) {
        // B
         System.out.println("(B)" +  this.getClass().getSimpleName() + "(" + type + ") hit with FireFlower (" + ff + ")");
     }
